...
-- The ON CONFLICT SET list is generated from that same column list by
-- upsertAll(), so the column list is the one place to update - but omit it
-- there and the column stays at its default forever.
...
-- Rebuild generation stamp (2026-08-27, perf/neon-compute-cost). persistSnapshot
-- now UPSERTs holder_metrics instead of DELETE+INSERT, so it needs a way to find
-- rows the current snapshot did not touch. Every upsert writes asOf here; the
-- prune then deletes `WHERE updated_at < asOf`. The default is deliberately in
-- the past so rows written before this column existed count as untouched and get
-- refreshed on the first run rather than surviving forever.
ALTER TABLE holder_metrics ADD COLUMN IF NOT EXISTS updated_at TIMESTAMPTZ NOT NULL DEFAULT 'epoch';
CREATE INDEX IF NOT EXISTS holder_metrics_stale_idx ON holder_metrics (asset, updated_at);
