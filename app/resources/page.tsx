    ...
    <div className="space-y-6">
      ...
      <Card title="Verified contract addresses">
        ...
        <div className="space-y-4">
          {VERIFIED_ADDRESSES.map((row) => (
            <div key={row.address} className="rounded-lg border border-[var(--border)] bg-black/20 p-3">
              <div className="mb-1 flex flex-wrap items-center gap-2">
                <span className="font-medium">{row.label}</span>
                <span className="text-xs text-neutral-500">{row.standard}</span>
                <Link
              ...
              {row.note && <p className="mt-2 text-xs text-neutral-500">{row.note}</p>}
            </div>
          ))}
        </div>
      </Card>
        ...
        <ul className="space-y-3">
          {MARKET_LINKS.map((l) => (
            <li key={l.label}>
              ...
              <p className="text-sm text-neutral-400">{l.blurb}</p>
            </li>
          ))}
        </ul>
      </Card>
      ...
      <Card title="Glossary">
        <dl className="grid gap-4 sm:grid-cols-2">
          {GLOSSARY.map((g) => (
            <div key={g.term}>
              <dt className="text-sm font-medium text-neutral-200">{g.term}</dt>
              <dd className="mt-0.5 text-sm text-neutral-400">{g.definition}</dd>
            </div>
          ))}
        </dl>
      </Card>
