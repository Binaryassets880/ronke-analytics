...
- **KTD-9 - Keyless in v1; API keys and per-key limits deferred behind a flag.** With KTD-1 caching, the marginal Neon cost of an anonymous public API is near zero, and requiring keys on day one is the fastest way to get zero integrations. Protection in v1 is: CDN caching, a hard `addresses` cap, no unbounded pagination (leaderboard `limit` <= 100), and `MAX_ROWS` on the full dump. An `api_keys` table plus per-key quotas becomes worthwhile only if someone needs uncached or high-volume access - U7 sketches it so the v1 contract does not have to change to add it.

  **CORRECTED 2026-08-11.** This decision originally also named "Vercel Firewall rate-limit rules configured in the dashboard" as the no-code escalation path. That was written before the hosting was checked: the project is on Vercel's **Hobby** tier (`binaryassets-projects`), where Firewall custom rules and rate limiting are Pro+ features. **There is no dashboard rate limit available.** The in-code caps above are therefore not the first line of defence, they are the only one. If abuse becomes real the options are upgrade to Pro or build U7 - do not plan around a Firewall rule that cannot be created. This also raises the stakes on KTD-1: on Hobby, an edge cache HIT is served without invoking a function, so caching protects the plan's 1M monthly function-invocation budget in addition to Neon's transfer allowance.
...
- **Approach:** Not built in v1 (KTD-9). NOTE the 2026-08-11 correction: the Vercel Firewall rate-limit rules this unit originally deferred to are **not available on the project's Hobby tier**, so there is no no-code escalation path. With KTD-1 caching the in-code caps are likely sufficient indefinitely; if they are not, the choice is upgrading to Pro (unlocks Firewall rules) or building this unit. If keys become necessary: an `api_keys (key_hash, label, owner, tier, created_at, revoked_at)` table, `Authorization: Bearer` checked in `lib/api/keys.ts`, keyed requests bypassing the address cap and unlocking a POST batch of up to 500. Design the v1 responses now so adding this changes no existing shape - it only relaxes limits.
