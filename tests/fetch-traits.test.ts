...
import { fetchTraits, type MetadataFetcher } from "@/scripts/fetch-traits";
...
function fakeFetcher(byToken: Record<string, MoralisAttribute[] | null>): MetadataFetcher {
  return async (tokenId: string) => {
    const attrs = byToken[tokenId];
    if (attrs === null || attrs === undefined) return null; // 404 / missing metadata
    return { attributes: attrs, imageUrl: `ipfs://${tokenId}` };
  };
}

describe("fetchTraits (U10, direct metadata fetch)", () => {
  it("fetches per token, counts revealed supply, flags unrevealed, persists rarity", async () => {
    ...
    const fetcher = fakeFetcher({
      ...
      "3": null, // metadata missing -> unrevealed
    });
    const res = await fetchTraits(sql, fetcher, new Date("2026-07-05T00:00:00Z"));
  ...
  it("resumes: skips token_ids already present in nft_traits", async () => {
    ...
    const fetcher: MetadataFetcher = async (tokenId) => {
      calledFor.push(tokenId);
      return { attributes: [attr("Body", "Ronke")], imageUrl: null };
    };
    await fetchTraits(sql, fetcher, new Date("2026-07-05T00:00:00Z"));
